...
#%%
#%%
Tri_dat_fast = adjust_time_speed(temp_dat, 3/4)
Tri_dat_slow = adjust_time_speed(temp_dat, 3/2)
#%%
#%%
...
#%%
#%%
print(Tri_dat[ 'dif_time' ][:8])
print(Tri_dat_fast[ 'dif_time' ][:8])
print(Tri_dat_slow[ 'dif_time' ][:8])
#%%
#%%
Train_set_actual = Train_set.copy()
...
#%%
# speeds = ['actual', 'fast', 'slow']
#%%
train_time_dfs = {
    'Train_set_actual': Train_set_actual.copy(),
    'Train_set_fast': Train_set_fast.copy(),
    'Train_set_slow': Train_set_slow.copy()
}
...
for key, df in train_time_dfs.items():
   ...
   y_dfs[key] = y.drop(nanX_positions).reset_index(drop=True).copy()
...
for key in train_time_dfs.keys():
...
param_grid = [{
    ...
    'lstm_units': [20, 30], # higher than 30
    'dropout_rate': [0, 0.1], 
...
#  0.9872
#%%
checkpoint_filepath = 'lstm_modI19Time_callback.h5'
...
history = model.fit(x=[XTV_past_Alltime, XTV_future_Alltime], y=yTV_Alltime,
                        validation_data=([XTV_past_Alltime, XTV_future_Alltime], yTV_Alltime),
                    #class_weight=class_weights_dict,
...
#%%
#%%
I19Time_model = load_model('lstm_modI19Time_callback.h5')
#%%
y_pred = (I19Time_model.predict([XTV_past_Alltime, XTV_future_Alltime]) > 0.5 ).astype(int)
accuracy = accuracy_score(yTV_Alltime,  y_pred)
f1 = f1_score(yTV_Alltime, y_pred, average='weighted')
# val_loss = history.history['val_loss'][-1]
print('Train set performance measures:')
...
for ind, key in enumerate(train_time_dfs.keys() ):
      
    size = int(len(y_pred)/3)
    temp_pred = y_pred[ind*size:(ind+1)*size]
...
for r in range(1,7):
    ...
    pred = (I19Time_model.predict([datpast, datfuture]) > 0.5 ).astype(int)
    dat['pred_lstm_time'] = np.nan
    dat.loc[~dat.index.isin(nanX_positions), 'pred_lstm_time'] = pred
    
    lstmpreds =  dat['pred_lstm_time'].values
    
    rig_data = datasets[f'rig_data{r}'].copy()
    rig_data['pred_lstm_time'] = lstmpreds
...
#%%
#%%
with open('UWB_datasets_19lstm_time_pred.pkl', 'wb') as f:
    pickle.dump(datasets, f)
#%%
with open('train_time_dfs.pkl', 'wb') as f:
    pickle.dump(train_time_dfs, f)

with open('ypred_TV_dict.pkl', 'wb') as f:
    pickle.dump(ypred_TV_dict, f)
    
    # here
#%%
%matplotlib qt 
animation_fun_pred(datasets['rig_data1'], col = 'pred_lstm_time', pace = 0.5, testing = False)
#%%
animation_fun_pred(datasets['rig_data2'], col = 'pred_lstm_time', pace = 0.15, testing = False)
#%%
animation_fun_pred(datasets['rig_data3'], col = 'pred_lstm_time', pace = 0.15, testing = False)
#%%
animation_fun_pred(datasets['rig_data4'], col = 'pred_lstm_time', pace = 0.15, testing = False)
#%%
animation_fun_pred(datasets['rig_data5'], col = 'pred_lstm_time', pace = 0.15, testing = False)
#%%
animation_fun_pred(datasets['rig_data6'], col = 'pred_lstm_time', pace = 0.01, testing = False, recent = 0.2, atatime = 0.1)
#%%
#%%
...
#%%
valid_y_pred = (I19Time_model.predict([Xvalid_past, Xvalid_future]) > 0.5 ).astype(int)
#%%
...
def get_feature_importance(j, n):
  ...
  for i in range(n):
    ...
    y_pred_ = (I19Time_model.predict([X_pasty, X_futuro]) > 0.5 ).astype(int)
...
for j in range(Xvalid_past.shape[2]):
  f_j = get_feature_importance(j, 30)
  f.append(f_j)
# Plot
