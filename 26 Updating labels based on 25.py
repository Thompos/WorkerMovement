...
#%%
import pickle
with open('UWB_datasets_24.pkl', 'rb') as f:
    datasets = pickle.load(f)
#%%
with open('intervals2check.pkl', 'rb') as f:
    intervals2check = pickle.load(f)
    
#%%
sorted_intervals = sorted(intervals2check, key=lambda x: x[0])
#%%
%matplotlib inline
#%%
Train_set = datasets['rig_data6'].reset_index(drop=True).copy()
#%%
na_color = 'gray'
zero_color = 'red'
nonzero_color = 'green'
#%%
starts_nan = [18098,18334,18367,18617,18620,18673,18794,18843,18844,18885,18883,19868,19904,19917,20791,20801,
              20815,21410,21824,21846,21827,21923,21954,23770,28222,28240,28251,32912,32966,33108,33205,33316,
              33435,33485,33747,34053,34169,35712,35737,35947,37245,37396,39230,40622,40874,41329,41402,43045,
              43070,43086,43308,43350,43608,43709,43971,44047,44126,46470,47096,50118,51543,56262,
              58107,58320,58642,58694,62576,68908,69027,69534,69556,69763,69804,69884,70460,75189,
              75253,75544]
ends_nan =   [18103,18349,18375,18627,18628,18701,18804,18848,18846,18893,18893,19888,19907,19930,20796,20806,
              20824,21414,21834,21864,21910,21933,21966,23771,28232,28246,28283,32946,32998,33118,33229,33346,
              33447,33495,33752,34111,34232,35722,35758,35953,37256,37410,39275,40632,40888,41364,41425,43051,
              43075,43106,43315,43361,43620,43739,44141,44061,44131,46472,47110,50158,51631,56272,
              58112,58360,58657,58714,62580,68911,69034,69541,69596,69793,69839,69919,70465,75192,
              75268,75571]
##
starts0 = [18272,18804,18893,19019,18994,19907,19911,19930,21380,21801,39536,39587,43278,43363,43678,43771,
           43991,47133,56282,58658,70141,70619,73970,75268,75637]
ends0 =   [18334,18839,18973,19034,19044,19918,19917,19958,21386,21824,39587,39633,43305,43371,43688,43851,
           44047,47182,57272,58694,70460,70754,74190,75303,75749]
##
starts1 = [23823,43560,43859,43901,43951,44061,47110,51387,57512,57834,58092]
ends1 =   [23827,43605,43881,43941,43959,44111,47133,51449,57532,57854,58107]
##
#%%
interval = sorted_intervals[403]

case = interval[0] # DO NOT CHANGE!
start = 58694 # case - 0
end = 58714 # case + 103
...
plt.scatter(Train_set['X'][start:end], Train_set['Y'][start:end],
            marker = 'o', s = 15.5, c=colors, alpha = 0.5)
plt.show()
print(f' ################################# DO NOT COPY {interval} DO NOT COPY')
print(colors)
print(start)
print(end)
#%%
...
#%%
### note that this snippet doesn't always work for some unkown reason, check that they have changed to nan above
for start, end in zip(starts_nan, ends_nan):  
...
#%%
#%%
selected_columns = [ 'difX', 'difY', 'dif_time', 'ed_dif', 'difSp1', 
                    'ed_dif_2', 'XY_spread2f', 'XY_spread2p',
                     'dif_angle1_abs', 'centroid_dist2p', 'centroid_dist2f' ]
other_columns = [ 'Xdiff_1', 'Ydiff_1', 'Xdiff_2', 'Ydiff_2', 'dif_angle_abs']
#%%
window_size = 40
...
def calculate_spread_dists(data):
    ...
    centroid_x = data['X'].median()
    centroid_y = data['Y'].median()
...
for rig in range(1,7):
    ...
    rig_data['difSp1'] = (rig_data['Xdiff_1']**2 + rig_data['Ydiff_1']**2)**0.5
    
    spreads2p = []
    centroid_dists2p = []
    for j in range(len(rig_data)):
        timestamp = rig_data.index[j]
        data_in_td2p = rig_data.loc[timestamp - td2 : timestamp ]
        spreads, centroid_dist = calculate_spread_dists(data_in_td2p)
        spreads2p.append(spreads)
        centroid_dists2p.append(centroid_dist)
        
    rig_data['XY_spread2p'] = spreads2p
    rig_data['centroid_dist2p'] = centroid_dists2p
    
    spreads2f = []
    centroid_dists2f = []
    for j in range(len(rig_data)):
        timestamp = rig_data.index[j]
        data_in_td2f = rig_data.loc[timestamp : timestamp + td2 ]
        spreads, centroid_dist = calculate_spread_dists(data_in_td2f)
        spreads2f.append(spreads)
        centroid_dists2f.append(centroid_dist)
        
    rig_data['XY_spread2f'] = spreads2f
    rig_data['centroid_dist2f'] = centroid_dists2f
    
        
    datasets[f'rig_data{rig}'] = rig_data.copy()
    
#%%
datasets['rig_data6']['move_cat'] = Train_set['move_cat'].values
#%%
#%%
...
def calc_variables(df, augmented = True):
    ...
    for j in range(len(df)):
        ...
        data_in_td2p = df.loc[timestamp - td2 : timestamp ]
        spreads, centroid_dist = calculate_spread_dists(data_in_td2p)
        spreads2p.append(spreads)
        centroid_dists2p.append(centroid_dist)
        
    df['XY_spread2p'] = spreads2p
    df['centroid_dist2p'] = centroid_dists2p
    
    spreads2f = []
    centroid_dists2f = []
    for j in range(len(df)):
        timestamp = df.index[j]
        data_in_td2f = df.loc[timestamp : timestamp + td2 ]
        spreads, centroid_dist = calculate_spread_dists(data_in_td2f)
        spreads2f.append(spreads)
        centroid_dists2f.append(centroid_dist)
        
    df['XY_spread2f'] = spreads2f
    df['centroid_dist2f'] = centroid_dists2f
...
#%%
#%% 
with open('datasets_fast_26_NV.pkl', 'wb') as f:
    pickle.dump(datasets_fast, f)
 
with open('datasets_slow_26_NV.pkl', 'wb') as f:
...
#%%
import joblib
joblib.dump(scaler, 'I26_time_NVs_scaler.pkl')
#%%
XpastTV_dict = {}
XfutureTV_dict = {}
y_TV_dict = {}
...
#%%
#%%
#********************* you might need to move this into the routine due to changes in python **************************************
early_stopping = EarlyStopping(monitor='val_loss', patience=6, restore_best_weights=True)
#%%
#%% 
param_grid = {
    'hidden_layers': [2], # 
    'dense_units': [15], # lower than 25.
    'lstm_units': [55], # higher than 40
...
n_splits = 3
fold_size = len(Xtrain_past_Alltime) // n_splits
...
for i in range(n_splits):
    ...
    if i == n_splits - 1:
        # For the last fold, include the remaining data
        end_idx = len(Xtrain_past_Alltime)

    test_idx = np.arange(start_idx, end_idx)
    train_idx = np.setdiff1d(np.arange(len(Xtrain_past_Alltime)), test_idx)
        ...
        for dropout_rate in param_grid['dropout_rate']:
            for lstm_units in param_grid['lstm_units']:
                ...
                for fold, (train_idx, test_idx) in enumerate(zip(train_indices, test_indices)):
                    ...
                    X1_train_fold, X1_test_fold = Xtrain_past_Alltime[train_idx], Xtrain_past_Alltime[test_idx]
                    X2_train_fold, X2_test_fold = Xtrain_future_Alltime[train_idx], Xtrain_future_Alltime[test_idx]
                    y_train_fold, y_test_fold = Ytrain_Alltime[train_idx], Ytrain_Alltime[test_idx]
                    ...
                    history = model.fit(x=[X1_train_fold, X2_train_fold], y=y_train_fold,
                                            validation_data=([X1_test_fold, X2_test_fold], y_test_fold),
                                         #class_weight=class_weights_dict,
                                         shuffle = True, 
                                          epochs=60, batch_size=32, verbose=1, callbacks=[early_stopping])
                    
                    y_pred = (model.predict([X1_test_fold, X2_test_fold]) > 0.5).astype(int) 
                ...
                params.append([hidden_layers, dense_units, lstm_units, dropout_rate])
...
#  0.9818
#%%
checkpoint_filepath = 'lstm_modI26TimeNV_CV_callback.h5'
...
history = model.fit(x=[Xtrain_past_Alltime, Xtrain_future_Alltime], y=Ytrain_Alltime,
                        validation_data=([Xvalid_past_Alltime, Xvalid_future_Alltime], Yvalid_Alltime), shuffle = True,
                    #class_weight=class_weights_dict,
...
#%%
#%%
I26Time_model = load_model('lstm_modI26TimeNV_CV_callback.h5')
#%%
y_pred = (I26Time_model.predict([Xvalid_past_Alltime, Xvalid_future_Alltime]) > 0.5 ).astype(int)
accuracy = accuracy_score(Yvalid_Alltime, y_pred)
f1 = f1_score(Yvalid_Alltime,  y_pred, average='weighted')
# val_loss = history.history['val_loss'][-1]
print('Test set performance measures:')
print(f1)
#print(val_loss)
print(accuracy)
#%%
y_pred_all = (I26Time_model.predict([XTV_past_Alltime, XTV_future_Alltime]) > 0.5 ).astype(int)
accuracy = accuracy_score(yTV_Alltime, y_pred_all)
f1 = f1_score(yTV_Alltime,  y_pred_all, average='weighted')
# val_loss = history.history['val_loss'][-1]
print('Test set performance measures:')
print(f1)
#print(val_loss)
...
for ind, key in enumerate(train_time_dfs_alt.keys() ):
    ...
    temp_pred = y_pred_all[count:(count+size)]
    count += size
...
def model_predictions(dataframes):
    ...
    for r in range(1,7):
        ...
        pred = I26Time_model.predict([Xpast, Xfuture])
        pred_int = (pred >= 0.5)*1
        dat['pred_26_NV'] = np.nan
        dat['pred_prob26_NV'] = np.nan
        dat.loc[~dat.index.isin(nanX_positions), 'pred_prob26_NV'] = pred
        dat.loc[~dat.index.isin(nanX_positions), 'pred_26_NV'] = pred_int
        
        lstmpreds = dat['pred_26_NV'].values
        
        rig_data = dict_data[f'rig_data{r}'].copy()
        rig_data['pred_26_NV'] = lstmpreds
        rig_data['pred_prob26_NV'] = dat['pred_prob26_NV'].values
...
#%%
with open('datasets_pred_26_NV.pkl', 'wb') as f:
    pickle.dump(datasets_pred, f)
    
with open('datasets_fast_pred_26_NV.pkl', 'wb') as f:
    pickle.dump(datasets_fast_pred, f)
    
with open('datasets_slow_pred_26_NV.pkl', 'wb') as f:
...
#%%
%matplotlib qt 
animation_fun_pred(datasets['rig_data1'], col = 'pred_26_NV', pace = 0.25, testing = False)
#%%
animation_fun_pred(datasets['rig_data2'], col = 'pred_26_NV', pace = 0.15, testing = False)
#%%
animation_fun_pred(datasets_pred['rig_data3'], col = 'pred_26_NV', pace = 0.15, testing = False)
#%%
animation_fun_pred(datasets_pred['rig_data4'], col = 'pred_26_NV', pace = 0.15, testing = False)
#%%
animation_fun_pred(datasets_pred['rig_data5'], col = 'pred_26_NV', pace = 0.15, testing = False)
#%%
animation_fun_pred(datasets_pred['rig_data6'], col = 'pred_lstm26_NV', pace = 0.01, testing = False, recent = 0.2, atatime = 0.1)
#%%
#%%
...
valid_y_pred = (I26Time_model.predict([Xvalid_past, Xvalid_future]) > 0.5 ).astype(int)
#%%
...
def get_feature_importance(j, n):
  ...
  for i in range(n):
    ...
    y_pred_ = (I26Time_model.predict([X_pasty, X_futuro]) > 0.5 ).astype(int)
